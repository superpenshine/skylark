...
def ConvBlock(fan_in, fan_out, stride=1, bias=False, kernel=(3, 19)):
    '''
    3x3 convolution with padding
    '''
    # return nn.Conv2d(fan_in, fan_out, kernel_size=3, stride=stride, bias=bias)
    return nn.Conv2d(fan_in, fan_out, kernel_size=kernel, stride=stride, bias=bias)
...
class ResNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = ConvBlock(2*4, 512, bias=True)
...
class DoubleConv(nn.Module):
    ...
    def __init__(self, fan_in, fan_out, **kwargs):
        ...
        self.conv1 = ConvBlock(fan_in, fan_out, **kwargs)
...
class UNet(nn.Module):
    ...
    def _make_layer(self, block, fan_out, cat=0, **kwargs):
        ...
        if cat != 0:
            self.fan_in += cat
        layer = block(self.fan_in, fan_out, **kwargs)
        self.fan_in = fan_out
